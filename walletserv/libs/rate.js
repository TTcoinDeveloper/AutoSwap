...
function usdPrice(currency, callback) {
        var pair = 'BTC' + '-' + config.currency
        jsonGet('https://bittrex.com/api/v1.1/public/getticker?market=' + pair, function(err, data) {
                if (err) {
                        callback(err);
                } else {
                        if (isset(data) && isset(isset(data.result.Bid))) {
                          console.log(data.result.Bid);
                                callback(false, data.result.Bid, currency);
                        } else {
                                callback(data);
                        }
